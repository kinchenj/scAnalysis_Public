...
subtitle: 'Epithelial cell analysis'
...
The full gene count table (GSE95435\_P150057\_full\_gene\_count\_table.txt) is required and can be downloaded from GEO. This should be saved in the 'data' subdirectory of the cloned repository.
...
# Load data
phenoData <- read.table("data/P150057_sample_metadata.txt", sep="\t", quote= "", header = TRUE, check.names = TRUE)
assayData <- read.table("data/GSE95435_P150057_full_gene_count_table.txt", sep="\t", quote= "", header = TRUE, check.names = TRUE)
...
tsne_opts <- list(perplexity = 15, max_iter = 2000)
...
set.seed(tsne_seed)
cell_tSNE <- f_eval ( ~ Rtsne(x, dims = 2, pca = FALSE, theta = 0.01, check_duplicates = FALSE, uqs(tsne_opts)), data = list(x = pcs$x[,1:tsne_dims]))
...
plotPCAclusters(ndims = 2, size_by = "detected_genes") + scale_size_continuous(limits=c(0,NA))
...
vio_genes <- c("MKI67","MUC2","SLC26A3")
